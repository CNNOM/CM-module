...
final class WarehouseStockService
{
        ...
        try {
            foreach ($basket as $basketItem) {
                ...
                $changes[] = ['stores' => array_map(
                    static fn(array $store): array => ['id' => (int)$store['ID'], 'amount' => (float)$store['AMOUNT'],
                    ],
                    $stores
                )];
    ...
    /** Восстанавливает складские остатки после ошибки заказа. */
    public static function restore(array $changes): void
    {
        foreach ($changes as $change) {
            foreach ($change['stores'] as $store) {
                \CCatalogStoreProduct::Update($store['id'], ['AMOUNT' => $store['amount']]);
            }
        }
    }
    ...
    {
        $stores = [];
        $result = \CCatalogStoreProduct::GetList(['ID' => 'ASC'], ['PRODUCT_ID' => $productId], false, false, ['ID', 'AMOUNT']);
    ...
    /** Списывает количество товара по складам последовательно. */
    private static function decreaseStores(array $stores, float $quantity, int $productId): void
    {
        ...
        foreach ($stores as $store) {
            ...
            if ($decrease > 0 && !\CCatalogStoreProduct::Update((int)$store['ID'], ['AMOUNT' => (float)$store['AMOUNT'] - $decrease])) {
