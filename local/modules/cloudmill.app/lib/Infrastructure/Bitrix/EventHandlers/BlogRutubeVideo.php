...
namespace CloudMill\App\Infrastructure\Bitrix\EventHandlers;

use Bitrix\Main\SystemException;
use CFile;
use CIBlockElement;
use CloudMill\App\Integrations\Rutube\RutubeVideoDto;
use CloudMill\App\Infrastructure\Logging\ExceptionHandler;
use CloudMill\App\Infrastructure\Bitrix\Wrappers\IblockManager;
use CloudMill\App\Integrations\Rutube\RutubeClient;
...
final class BlogRutubeVideo
{
    ...
    {
        return !self::$handlerDisallow && $fields['IBLOCK_ID'] == IblockManager::getID(self::VIDEO_BLOG_IBLOCK_CODE);
    }

    private static function getElementData(int $elementId): array
    {
        return CIBlockElement::GetList(
            arFilter: ['ID' => $elementId, 'IBLOCK_ID' => IblockManager::getID(self::VIDEO_BLOG_IBLOCK_CODE)],
    ...
    {
        $videoDto = RutubeClient::getVideoByLink($elementData['PROPERTY_' . self::PROPERTY_RAW_LINK . '_VALUE']);
        if (!$videoDto) return;
    ...
    private static function updateElement(int $elementId, RutubeVideoDto $videoDto): void
    {
        ...
        if (!$isUpdated) {
            ExceptionHandler::handle(new SystemException($elem->getLastError()));
        }
    }
