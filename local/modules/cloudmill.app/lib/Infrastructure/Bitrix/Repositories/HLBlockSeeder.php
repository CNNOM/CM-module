...
class HLBlockSeeder
{
    ...
    {
        $hlBlock = HighloadBlockManager::getInfo(filter: ['NAME' => $name])[0];
    ...
    private static function createHLBlockRaw($name, $tableName, $displayName = ''): array
    {
        ...
        $hlBlock = HighloadBlockManager::getInfo(filter: ['ID' => $result->getId()])[0];
    ...
    private static function createFieldsIfNotExists(string $hlCode, ?array $fields): void
    {
        ...
        $errors = [];
        $propsInfo = HighloadBlockManager::getFieldsInfo($hlCode);
    ...
    {
        $entityId = HighloadBlockManager::getEntityIDByCode($hlCode);
        if (!$entityId) {
            return false;
        }
    ...
    {
        $hl = HighloadBlockManager::getEntity($hlCode);
        if (!$hl) {
    ...
    {
        $fieldsInfo = HighloadBlockManager::getFieldsInfo($hlCode);
            ...
            switch ($field['USER_TYPE_ID']) {
                case 'enumeration':
                    ...
                    if ($field['MULTIPLE'] !== 'Y') {
                        ...
                        $values[$key] = HighloadBlockManager::getEnumList($key, $value)[0]['ID'] ?? '';
                        break;
                    }

                    $value = is_array($value) ? $value : [$value];
                    $enums = HighloadBlockManager::getEnumList($key, $value, 'VALUE');
