...
namespace CloudMill\App\Infrastructure\Bitrix\Repositories;
...
use CloudMill\App\Infrastructure\Bitrix\Data\DataHelper;
...
class IBlockSeeder
{
        ...
        int     $sort = 500)
    {
        ...
        $code = $codeToFind;
        $name ??= $codeToFind;
        $iblock = IblockManager::getIBlockList(filter: ['CODE' => $code])[0] ?? null;
    ...
    {
        $item = IblockManager::getIBlockType($code);
        if ($item) {
    ...
    {
        try {
            ApplicationContext::getDB()->startTransaction();
            ...
            ApplicationContext::getDB()->commitTransaction();
        } catch (Exception $e) {
            ApplicationContext::getDB()->rollbackTransaction();
            throw $e;
        }

        IblockManager::getID($code, resetCache: true); // обновление кэша списка ИБ

        return IblockManager::getIBlockList(filter: ['CODE' => $code])[0];
    }
    ...
    public static function createIBlockPropsIfNotExists(int|string $iblockCode, ?array $propsList): void
    {
        ...
        $iblockID = IblockManager::getID($iblockCode);
        if (!$iblockID) {
    ...
    public static function firstOrCreateSection($iblockCode, $fields = []): ?array
    {
        ...
        $fields['CODE'] ??= DataHelper::translit($fields['NAME']);

        $section = IblockManager::getSectionList(filter: ['CODE' => $fields['CODE'], 'IBLOCK_ID' => $iblockID])[0];
        if (!$section) {
    ...
    private static function createSection($iblockCode, $fields): array
    {
        ...
        return IblockManager::getSectionList(filter: ['CODE' => $fields['CODE'], 'IBLOCK_ID' => $iblockID])[0];
    }
    ...
    public static function updateSectionFieldsByName($iblockCode, string $sectionName, array $fields): ?array
    {
        ...
        $section = IblockManager::getSectionList(
    ...
    public static function firstOrCreateElement($iblockCode, $fields): array
    {
        ...
        $item = IblockManager::getList(filter: ['CODE' => $fields['CODE'], 'IBLOCK_ID' => $iblockID])[0];
    ...
    private static function createElement($iblockCode, $fields)
    {
        ...
        return IblockManager::getList(filter: ['CODE' => $fields['CODE'], 'IBLOCK_ID' => $iblockID])[0];
    }
    ...
    private static function formatPropByType($propInfo, $val): mixed
    {
        ...
        switch ($propInfo['PROPERTY_TYPE']) {
            ...
            case 'L':
                ...
                if ($propInfo['MULTIPLE'] !== 'Y') {
                    return IblockManager::getEnumIDByValue($propInfo['IBLOCK_ID'], $propInfo['CODE'], $val);
                }
                return array_map(fn($v) => IblockManager::getEnumIDByValue($propInfo['IBLOCK_ID'], $propInfo['CODE'], $v), $val);
        }
    ...
    private static function formatStrProp(array $propInfo, $val): string|array
    {
        ...
        if (is_string($val)) {
            return DataHelper::fixCopiedHtmlText($val);
        }

        if ($propInfo['WITH_DESCRIPTION'] === 'Y' && !empty($val['VALUE'])) {
            $val['VALUE'] = DataHelper::fixCopiedHtmlText($val['VALUE']);

            if (!empty($val['DESCRIPTION'])) {
                $val['DESCRIPTION'] = DataHelper::fixCopiedHtmlText($val['DESCRIPTION']);
            }
        }
    ...
    {
        return IblockManager::getPropsList($iblockID, preferBy: 'CODE', select: ['*']);
    }
    ...
    private static function replicateItem($elementID, $cnt): void
    {
        ...
        $item = IblockManager::getListWithProps(filter: ['ID' => $elementID, 'IBLOCK_ID' => $iblockID])[$elementID];
        $iblockCode = IblockManager::getIBlockList(filter: ['ID' => $iblockID])[0]['CODE'] ?? '';
        ...
        foreach (range(1, $cnt) as $i) {
            ...
            $fieldsToSave['CODE'] = DataHelper::translit($fieldsToSave['NAME']) . '-' . uniqid();
