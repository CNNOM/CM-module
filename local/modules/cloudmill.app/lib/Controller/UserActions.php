...
use Cloudmill\App\Helpers\UserActionsHelper;
use Cloudmill\App\Services\CatalogCompareService;

final class UserActions extends Controller
{
    ...
    public function handleAction(
        ...
        if (!UserActionsHelper::isValidAction($userAction)) {
            $this->addError(new Error('Unsupported user action'));
            return null;
        }

        $storage = UserActionsHelper::getStorage($userAction);
        ...
        if ($userAction === UserActionsHelper::ACTION_COMPARE) {
        ...
        if ($order === 'delete') {
            ...
            if (
                $userAction === UserActionsHelper::ACTION_COMPARE
