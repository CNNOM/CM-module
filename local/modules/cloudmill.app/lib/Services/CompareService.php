...
final class CompareService
{
    ...
    public static function share(): array
    {
        ...
        $saved = HLBlockHelper::getList(
            self::HL_SHARE,
            ['select' => ['ID', 'UF_HASH'], 'filter' => ['UF_HASH' => $hash], 'limit' => 1]
        )[0] ?? null;

        $entity = HLBlockHelper::getEntity(self::HL_SHARE);
        if (!$entity) {
            return [
                'success' => false,
                'code' => 'NOT_FOUND_HL',
    ...
    {
        $raw = (string)AppContext::getRequest()->getCookie(self::COOKIE_NAME);
        if ($raw === '') {
            return [];
        }
    ...
    private static function saveItems(array $items): void
    {
        ...
        AppContext::getResponse()->addCookie($cookie);
    }
